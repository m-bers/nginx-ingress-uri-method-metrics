...
5. Determine which resource you want to apply per-URI and per-method metrics to and open up the corresponding file with extension `.tmpl` For example, `$PROJECT_ROOT/internal/configs/version1/nginx-plus.ingress.tmpl` is the template file for the Ingress resource on NGINX Plus, and `$PROJECT_ROOT/internal/configs/version2/nginx-plus.virtualserver.tmpl` is the template file for the VirtualServer CRD resource on NGINX Plus.
